...
FLAGS <- flags(
  ...
  flag_integer("ageph_emb", 10),
  flag_integer("agecar_emb", 10),
  flag_integer("power_emb", 10),
  flag_integer("fuel_emb", 1),
  flag_integer("gender_emb", 1),
  flag_integer("use_emb", 1),
  flag_integer("latitude_emb", 10),
  flag_integer("longitude_emb", 10), #2++2+5+5+5+1+1+1+1+1 = 24
  
  
  ### LAYER 1
  
  flag_integer("dense_units1", 114),
  flag_numeric("dropout1", 0),
  flag_numeric("regularizer_l1_1", 0.00),
  flag_numeric("regularizer_l2_1", 0.000),
  flag_string("activation1", "relu"),
  
  
  ### LAYER 2
  
  flag_integer("dense_units2", 114),
  flag_numeric("dropout2", 0),
  flag_numeric("regularizer_l1_2", 0.00),
  flag_numeric("regularizer_l2_2", 0.000),
  flag_string("activation2", "relu"),
  
  ### LAYER 3
  
  flag_integer("dense_units3", 60),
  flag_numeric("dropout3", 0),
  flag_numeric("regularizer_l1_3", 0.0),
  flag_numeric("regularizer_l2_3", 0.000),
  flag_string("activation3", "relu"),
  
  ### LAYER 4
  
  flag_integer("dense_units4", 30),
  flag_numeric("dropout4", 0),
  flag_numeric("regularizer_l1_4", 0),
  flag_numeric("regularizer_l2_4", 0.000),
  flag_string("activation4", "relu"),
  
  ### LAYER 4
  
  flag_integer("dense_units5", 10),
  flag_numeric("dropout5", 0),
  flag_numeric("regularizer_l1_5", 0),
  flag_numeric("regularizer_l2_5", 0.000),
  flag_string("activation5", "relu"),
...
Latitude_embed = Latitude %>% 
  layer_embedding(input_dim = 97, output_dim = FLAGS$latitude_emb, input_length = 1, name = 'Latitude_embed') %>%
  keras::layer_flatten()
...
model_fr %>% compile(
  ...
  loss =  "binary_crossentropy",#function(y_true, y_pred) pDev(y_true, y_pred), #function(y_true, y_pred) pDev(y_true, y_pred)#function(y_true, y_pred) pDev(y_true, y_pred)
  metric = c("mae")) 
...
callbacks_list <- list(
  ...
  callback_tensorboard(
    log_dir = "C:/Machine Learning/Pricing Game/Pricing Game (UCL-ULB)/Tensorboard/Tensorbord_directory_frequency",
    histogram_freq = 1,
    embeddings_data = embedding_dat 
  )
)'
