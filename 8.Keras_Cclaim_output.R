...
data_embed <- training_set[training_set$Cclaim>0,]

########## EXPERT JUDGMENT ########## 

data_embed <-  filter(data_embed, Cclaim < 20000,  Nclaim > 0)

AvgCclaim <- data_embed$Cclaim/data_embed$Nclaim

data_embed <- cbind(AvgCclaim, data_embed)

data_embed <- data_embed[,-which(names(data_embed) %in% c("Nclaim", "Cclaim"))]
...
Data_embed[,PowerNN    :=as.integer(as.factor(cut2(Power,m=1000)))-1]
...
y_train = list(N = learn$AvgCclaim)
